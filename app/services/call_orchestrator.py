...
# Empty-input filter
# ═══════════════════════════════════════════════════════════════
# In v11, barge-in is OFF. Twilio's <Gather> only sends us speech AFTER
# the customer finishes speaking — so by the time anything reaches us,
# it's a real, complete utterance. We don't need the v9-era backchannel
# filter that was discarding "uh-huh" / coughs etc.; Twilio's STT
# handles that for us by simply not capturing brief noises during
# AI playback (since playback ends before Gather opens).
#
# The only thing we still need to gate is genuinely empty input —
# Twilio occasionally posts /process-speech with an empty SpeechResult
# when the Gather window opened but nothing was spoken before the
# silence timeout fired. We detect that and route to /silence-prompt.

def _is_empty_input(text: str) -> bool:
    """Return True if the utterance is just whitespace or punctuation."""
    s = (text or "").strip()
    if not s:
        return True
    # Strip all non-word characters; if nothing's left, it was just
    # noise/punctuation that STT included in the result.
    if not re.sub(r"[^\w]+", "", s):
        return True
    return False
...
class CallOrchestrator:
    ...
    async def handle_speech(
        ...
        # Empty input → route to /silence-prompt for the polite "are
        # you still there?" follow-up. Twilio sometimes fires Gather
        # with an empty SpeechResult when nothing was said before the
        # silence timeout.
        if _is_empty_input(speech):
            log.info("[%s] empty speech, routing to silence-prompt", sid)
            return twiml.listen_silent()

        # In v11 there's no barge-in, so a "prior reply still in flight"
        # situation is rare — the customer can only speak after the AI
        # finishes. But it CAN still happen if the customer talks during
        # the brief moment between AI audio ending and Gather opening.
        # We cancel the producer task defensively so any tail audio
    ...
    async def handle_silence_prompt(self, sid: str) -> str:
        ...
        if state._silence_count >= 2:
            ...
            return twiml.hangup_clean()
        # First silence — one warm "are you there?" before giving up.
        from app.core.config import get_settings
        base = get_settings().base_url
